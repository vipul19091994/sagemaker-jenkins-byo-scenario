      ...
      stage("TrainMonitor") {
          steps {
              script {
                  waitUntil {
                    def status = sh """ 
                    aws lambda invoke --function-name ${params.LAMBDA_CHECK_STATUS_TRAINING} --cli-binary-format raw-in-base64-out --payload "{"TrainingJobName": "${params.SAGEMAKER_TRAINING_JOB}-${env.BUILD_ID}"}" response.json
                    echo status
                    """
